...
fn drawAlphaRect(dst: []u8, stride: u16, x: u16, y: u16, width: u16, height: u16, alpha: u8) void { const stride_usize = @as(usize, stride); const x_usize = @as(usize, x); const y_usize = @as(usize, y); const width_usize = @as(usize, width); const height_usize = @as(usize, height); std.debug.assert(x_usize + width_usize <= stride_usize); std.debug.assert((y_usize + height_usize) * stride_usize <= dst.len); for (y..y + height) |yy| { const row = yy * stride_usize; for (x..x + width) |xx| dst[row + xx] = alpha; } }
...
fn drawAlphaQuadraticStroke(dst: []u8, w: u16, h: u16, p0: PointF, p1: PointF, p2: PointF, thickness: f64) void { const half = thickness / 2.0; const samples = 48; for (0..h) |yy| { for (0..w) |xx| { const px = @as(f64, @floatFromInt(xx)) + 0.5; const py = @as(f64, @floatFromInt(yy)) + 0.5; var min_d2: f64 = std.math.floatMax(f64); var i: usize = 0; while (i <= samples) : (i += 1) { const u = @as(f64, @floatFromInt(i)) / @as(f64, @floatFromInt(samples)); const inv = 1.0 - u; const cx = inv * inv * p0.x + 2.0 * inv * u * p1.x + u * u * p2.x; const cy = inv * inv * p0.y + 2.0 * inv * u * p1.y + u * u * p2.y; const dx = px - cx; const dy = py - cy; min_d2 = @min(min_d2, dx * dx + dy * dy); } const dist = @sqrt(min_d2); const coverage = std.math.clamp(half - dist + 0.5, 0.0, 1.0); if (coverage <= 0.0) continue; const alpha: u8 = @intFromFloat(@round(coverage * 255.0)); const off = yy * @as(usize, w) + xx; dst[off] = @max(dst[off], alpha); } } }
