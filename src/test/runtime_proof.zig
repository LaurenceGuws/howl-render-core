//! Responsibility: prove the render-core runtime owner chain.
...
test "renderer package surface remains available" {
    ...
    _ = root.Core.Metrics;
    _ = root.Core.RenderRuntime.Metrics;
...
test "render runtime owner behavior remains reachable" {
    var runtime = root.Core.RenderRuntime.init(std.testing.allocator);
    defer runtime.deinit();
    var snapshot = try root.Core.FrameSnapshot.init(std.testing.allocator, 2, 3);
    ...
    const source = root.Core.SourceView{
        .snapshot = &snapshot,
        .cols = 3,
        .rows = 2,
        .scrollback_count = 0,
...
test "render runtime stale retained-base validation stays explicit" {
    const submitted = root.Core.FramePipeline.SubmittedFrame{
        .token = .{ .snapshot_seq = 10, .dirty_epoch = 10, .geometry_epoch = 3, .damage_base_seq = 0, .damage_kind = .full },
        .target_epoch = 7,
        .content_valid = true,
    };
    const stale_base = root.Core.FramePipeline.PreparedFrame{
    ...
    const stale_target = root.Core.FramePipeline.PreparedFrame{
    ...
    try std.testing.expectEqual(root.Core.FramePipeline.SubmitValidation.stale_retained_base, root.Core.FramePipeline.validatePreparedFrame(stale_base, submitted));
    try std.testing.expectEqual(root.Core.FramePipeline.SubmitValidation.stale_target, root.Core.FramePipeline.validatePreparedFrame(stale_target, submitted));
}
