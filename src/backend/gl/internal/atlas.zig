...
const draw_pass_mod = @import("draw_pass.zig");
const c = c_api.c;
...
pub fn uploadTextSceneRaster(
    ...
    if (draw_pass_mod.hasCurrentContext()) try ensureAtlasTexture(self);
    if (draw_pass_mod.hasCurrentContext()) uploadSceneResidentSlots(self, scene);
    ...
    for (outputs) |output| {
        ...
        if (draw_pass_mod.hasCurrentContext()) _ = uploadAtlasSlot(self, slot);
        committed += 1;
    }
    return committed;
}
...
pub fn ensureAtlasTexture(self: anytype) !void {
    if (!draw_pass_mod.hasCurrentContext()) return;
    const max_slots = self.capabilities().max_atlas_slots;
